...
local function detectClient(force)
  ...
  if g_app then
    ...
    if ok and type(n) == "string" then
      local low = n:lower()
      if low:find("xiba") then
        signals.app = "xibat"
      elseif low:find("redemption") then
        signals.app = "otbr"
      end
    end
  ...
  if signals.app == "xibat"    then _clientType = ACL.ClientType.OTCV8;       ACL.clientName = "Xibat"
  elseif signals.file == "otbr" then _clientType = ACL.ClientType.OPENTIBIABR; ACL.clientName = "OpenTibiaBR"
